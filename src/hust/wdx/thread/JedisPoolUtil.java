...
public class JedisPoolUtil {
	...
	public JedisPoolUtil(String ip, int port) {
		if (pool == null) {
			...
			pool = new JedisPool(config, ip, port,100000,"yang");
		}
	}
	 ...
	 * @param port 端口
	 */
	public JedisPoolUtil(JedisPoolConfig config ,String ip, int port){
		if (pool == null) {
	 ...
	 * @param timeout 超时时间
	 */
	public JedisPoolUtil(JedisPoolConfig config ,String ip, int port ,int timeout){
		if (pool == null) {
	 ...
	 * @param pool 连接池对象
	 */
	public JedisPoolUtil(JedisPool pool){
		if (this.pool == null) {
			this.pool = pool;
		}
	}
